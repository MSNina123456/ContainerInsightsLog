...
init()
{
    echo -e "Preparing for log collection..."
    ...
    echo -e "Prerequistes check is done, all good"
}

ds_logCollection()
{
    echo -e "Collecting logs from ${ds_pod}..."
    ...
    if [[ $cmd == *"cannot access"* ]];then
        echo -e "${Red}/var/opt/microsoft not exist on ${ds_pod}${NC}"
    else
    ...
    if [[ $cmd == *"cannot access"* ]];then
        echo -e "${Red}/etc/fluent not exist on ${ds_pod}${NC}"
    else
    ...
    if [[ $cmd == *"cannot access"* ]];then
        echo -e "${Red}/etc/opt/microsoft/docker-cimprov not exist on ${ds_pod}${NC}"
    else
    ...
    echo -e "Complete log collection from ${ds_pod}!"
}

win_logCollection()
{
    echo -e "Collecting logs from ${ds_win_pod}, windows pod will take few minutes for log collection, please dont exit forcely..."
    ...
    echo -e "Complete log collection from ${ds_win_pod}!"
}
...
rs_logCollection()
{
    ...
    if [[ $cmd == *"cannot access"* ]];then
        echo -e "${Red}/var/opt/microsoft not exist on ${rs_pod}${NC}"
    else
    ...
    if [[ $cmd == *"cannot access"* ]];then
        echo -e "${Red}/etc/fluent not exist on ${rs_pod}${NC}"
    else
    ...
    if [[ $cmd == *"cannot access"* ]];then
        echo -e "${Red}/etc/opt/microsoft/docker-cimprov not exist on ${rs_pod}${NC}"
    else
    ...
    echo -e "Complete log collection from ${rs_pod}!"
}
...
other_logCollection()
{
    ...
    if [ -z "$deploy" ];then
        echo -e "${Red}there is not omsagent deployment, skipping log collection of deployment${NC}"
    else
    ...
    if [ -z "$config" ];then
        echo -e "${Red}configMap named container-azm-ms-agentconfig is not found, if you created configMap for omsagent, please use command to save your custom configMap of omsagent: kubectl get configmaps <configMap name> --namespace=kube-system -o yaml > configMap.yaml${NC}"
    else
        kubectl get configmaps $config --namespace=kube-system -o yaml > ${config}.yaml
    fi

    kubectl get nodes > node.txt
    echo -e "Complete onboard log collection!"
}
...
if [ -z "$ds_pod" ];then
	echo -e "${Red}daemonset pod do not exist, skipping log collection for daemonset pod${NC}"
else
    ds_logCollection
fi
...
if [ -z "$ds_win_pod" ];then
	echo -e "${Cyan} windows agent pod do not exist, skipping log collection for windows agent pod ${NC}"
else
    win_logCollection
fi
...
if [ -z "$rs_pod" ];then
	echo -e "${Red}replicaset pod do not exist, skipping log collection for replicaset pod ${NC}"
else
    rs_logCollection
fi
