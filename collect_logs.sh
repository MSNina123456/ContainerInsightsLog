...
other_logCollection()
{
    ...
    if [ -z "$config" ];then
        echo -e "${Red}configMap named container-azm-ms-agentconfig is not found, please use command to save your custom configMap of omsagent: kubectl get configmaps <configMap name> --namespace=kube-system -o yaml > configMap.yaml${NC}"
    else
...
init()
{
    ...
    cmd=`kubectl get node --namespace=kube-system | sed 1,1d | awk '{print $2}'`
    for node in $cmd
    do
